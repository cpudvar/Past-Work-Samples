...
class BoardClass(object):
	...
	# root
        ...
        """invalid values initializing heuristics"""
    ...
    def copyCTOR(self):
        ...
        newBoard.ManhattanDistance = self.ManhattanDistance
    ...
    def computeManhattan(self):
        """INPUT: board
	   OUTPUT: Manhattan Distance of board assigned to that board
	   
	   Description: given a board, this code calculates the
	   Manhattan Distance of that board using the goal board. 
	   
	   Manhatten Distance = (sum of misplaced distances for all tiles)
	"""
	
        sum = 0
	
        for tile in range(0, BoardClass.N ** 2):
            boardRow, boardCol = self.findBoardTile(tile)
            goalRow, goalCol = BoardClass.findGoalTile(tile)
            """finds the distance away between rows, adds to distance away
	       between columns
            """
            sum += abs(goalRow - boardRow) + abs(goalCol - boardCol)        
	
        self.ManhattanDistance = sum
